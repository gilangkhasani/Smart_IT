...
class m_sb_export extends CI_Model{
	...
	function saveBarang($doc_id)
	{
		...
		if($doc_id != NULL){
			...
			$this->db->where('doc_id',$doc_id);
			$this->db->delete('sb_doc_export_dt');
			...
			if ($this->input->post('barang_code')[0]==""){
				...
				for( $i = 0; $i < count($this->input->post('barang_code')); $i++ ){
					...
					$this->db->insert('sb_doc_export_dt', $data_arr);
				}
				$this->session->set_flashdata('berhasil', 'Data berhasil diubah');
			}
		}else {
			$id = $this->db->query('SELECT ISNULL(MAX(SUBSTRING(doc_id,9,6)),0) + 1 AS doc_id FROM sb_doc_export_hd');
			$doc_id = $id->row();
			...
			$this->db->insert('sb_doc_export_hd', $data);
			
			//$query = $this->db->query('SELECT MAX(doc_id) as doc_id FROM sb_doc_export_hd');
			//$row = $query->row();
	...
	function get_all_sb_export_count($jenis)
	{
		$this->db->where('status','UnFinish');
		$this->db->select('count(*) as jumlah');
		$this->db->from('sb_doc_export_hd');
		$this->db->where('jenis_doc',$jenis);
		$query = $this->db->get();
		return $query->row();
	}
	
	function get_all_sb_export_limit($jenis,$p,$u)
	{
		if(!$u){
			$u = 0;
			$p;
	...
	function getAllExport($jenis)
	{
		$query = $this->db->query("select a.*, b.supplier_name,
							(
							select count(*)
							from sb_doc_export_dt
							where doc_id=a.doc_id
	...
	function get_sb_export_hd($doc_id)
	{
		$this->db->where('doc_id',$doc_id);
		$query = $this->db->get('sb_doc_export_hd');
		return $query->row();
	}
	
	function get_sb_export_dt($doc_id)
	{
		$today = date('Y-m-d');
		...
		*/
		...
		");
		$this->db->from('sb_doc_export_dt a');
		$this->db->join('sb_barang b', 'a.barang_code=b.barang_code');
		$query = $this->db->get();
		return $query->result();
	}
	
	function del_sb_export($doc_id)
	{
		$this->db->where('doc_id',$doc_id);
		$this->db->delete('sb_doc_export_dt');
		
		$this->db->where('doc_id',$doc_id);
		$this->db->delete('sb_doc_export_hd');
	}
