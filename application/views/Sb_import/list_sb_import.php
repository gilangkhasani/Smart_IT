...
<head>
	...
	<script>
			...
			$("#formsearch").submit(function(){
				if($("#jenis").val() == 'all'){
					window.location.replace("<?php echo base_url()?>index.php/Sb_import/listing/<?php echo $this->uri->segment(3);?>");
				} else {
					...
					url: "<?php echo base_url();?>index.php/Sb_import/search/<?php echo $this->uri->segment(3);?>",
					data: x,
...
<body>
	...
	<div class="box box-info">
		<div class="box-header">
			<div class="box-title">
				<h2 style="font-size:23px;">List of Import <?php echo str_replace('_', ' ', $this->uri->segment(3));?></h2>
				<a class="btn btn-primary" href="<?php echo base_url()?>index.php/Sb_import/add/<?php echo $this->uri->segment(3)?>">
				<i class="fa fa-plus"></i> &nbsp New Import</a>
			...
			<form id="formsearch" style="margin-left:700px; margin-top:70px;" method="post" onsubmit="return false">
					...
					<select name="jenis" id="jenis">
						...
						<option value="doc_id">No. Dokumen</option>
						<option value="no_bukti">No. Penerimaan</option>
		...
		<table class="table table-hover" id="table-1" style="border-bottom:1px solid;border-top:1px solid;">
			...
			<tbody>
				<?php $tes=0 ?>
				<?php foreach($import as $import) { ?>
				<tr>
					<td><?php echo $import->no_doc?></td>
					<td><?php echo $import->tgl_doc?></td>
					<td><?php echo $import->no_bukti?></td>
					<td><?php echo $import->tgl_bukti?></td>
					<td><?php echo $import->supplier_code?></td>
					<td><?php echo $import->valas?></td>
					<td><a href="javascript:0" onClick="openWin('<?php echo base_url()?>index.php/Sb_import/lihat_detail/<?php echo $import->doc_id?>','popupdetail','600','500')"><?php echo $import->jumlah?></a></td>
					
					<!--
					<td><a class="btn btn-success btn-small" style="height:27px; font-size:11px;" onclick="return confirm('Are you sure ?')" href="<?php echo base_url()?>index.php/Sb_import/ubah_status/<?php echo $import->doc_id?>/<?php echo str_replace(' ', '_', $import->jenis_doc)?>">Finish</a></td>
					!-->
					<td><a onclick="modalEdit('<?php echo $import->doc_id?>');" class="btn btn-success btn-small" style="height:27px; font-size:10px;" data-toggle="modal" data-target="#myModal1">Finish</a></td>
					
					<td><a class="btn btn-primary btn-small" style="height:27px; font-size:11px;" href="<?php echo base_url()?>index.php/Sb_import/edit/<?php echo $this->uri->segment(3)?>/<?php echo $import->doc_id?>">Edit</a></td>
					
					<td><a class="btn btn-danger btn-small" style="height:27px; font-size:11px;" onclick="return confirm('Are you sure ?')" href="<?php echo base_url()?>index.php/Sb_import/delete/<?php echo $import->doc_id?>/<?php echo $this->uri->segment(3)?>">Delete</a></td>
	  ...
	  <div class="modal-dialog" style="width: 500px; margin-top:120px;">
		<div class="modal-content">
		  <div class="modal-header">
			...
			<h4 class="modal-title" id="myModalLabel"><i class="fa fa-hdd-o"></i> &nbsp Bukti Penerimaan Barang</h4>
		  </div>

		  <div class="modal-body">
			<div class="box-body">
				<form method="post" class="form-tes" action="<?php echo base_url()?>index.php/Sb_import/finish">
					<div>
					...
					<div>
						<label for="tgl_bukti">Tgl. Penerimaan</label>
						<div>
					  ...
					  <?php
							$jenis_doc = str_replace(' ', '_', $import->jenis_doc);
