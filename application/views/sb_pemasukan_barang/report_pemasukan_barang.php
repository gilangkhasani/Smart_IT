...
<head>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$('#nilai_barang').on( 'keyup', function () {
				table
					.columns( 12 )
					.search( this.value )
					.draw();
			...
			/*
			$("#type,#type2,#type3,#type4,#type5").change(function() {
				var types = $('input:checkbox:checked').map(function() {
					return '^' + this.value + '\$';
				}).get().join('|');
				//filter in column 0, with an regex, no smart filtering, no inputbox,not case sensitive
				table1.fnFilter(types, 1, true, false, false, false);
			});
			*/
		} );
	</script>
...
<body>	
	<h3> Laporan Pemasukan Barang per Dokumen</h3>
	
	<form action="<?php echo base_url()?>Importexcel/report_pemasukan" method="post" target="_blank">
		<div>
			<fieldset>
	...
	<br/>
	<table id="example" class="table table-hover" border="1px solid black" style="font-size:12.5px;">
		<thead>
			<tr>
				<th rowspan="2"><center>No</center></th>
				<th colspan="3"><center>Dokumen Pabean</center></th>
				<th colspan="2"><center>Bukti Penerimaan</center></th>
		...
		<tbody>
			...
			<tr>
				<td align="center";><?php echo $laporan->no;?></td>
