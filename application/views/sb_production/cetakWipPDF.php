...
<?php
	...
	$this->fpdf->Cell(3 ,1, 'Jumlah barang' , 1, 'LR', 'C');
	$this->fpdf->Cell(3.2,1, 'Keterangan' , 1, 'LR', 'C');
	foreach($wip as $data){
	$this->fpdf->Ln();
	$this->fpdf->SetFont('Times','',9);
	$this->fpdf->Cell(1 , 1, $data->no , 1, 'LR', 'C');
	...
	$this->fpdf->Cell(3 ,1, number_format($data->jumlah_barang,2) , 1, 'LR', 'C');
	$this->fpdf->Cell(3.2,1, $data->keterangan , 1, 'LR', 'C');
	}
	$this->fpdf->Output();
